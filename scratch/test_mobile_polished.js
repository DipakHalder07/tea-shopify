...
async function run() {
  ...
  await send('Runtime.evaluate', {
    expression: `(() => {
      document.querySelectorAll('[class*="cookie"], #cookie-bar, .cookie-banner').forEach(el => el.remove());
      const g = document.querySelector('.product__media-gallery');
  ...
  fs.writeFileSync(`${baseDir}\\pdp_live_mobile_final.png`, Buffer.from(shotMobile.data, 'base64'));
  console.log('Saved pdp_live_mobile_final.png');
