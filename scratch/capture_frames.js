...
async function run() {
  ...
  // Navigate to local shopify store
  ...
  for (let i = 0; i <= 10; i++) {
    ...
    if (snap && snap.data) {
      fs.writeFileSync(path.join(outDir, `frame_${i * 300}ms.png`), Buffer.from(snap.data, 'base64'));
    }
    const state = await send('Runtime.evaluate', {
      expression: `(() => {
        const items = Array.from(document.querySelectorAll('.pl-item')).map(it => ({
          ...
          imgNaturalWidth: it.querySelector('img') ? it.querySelector('img').naturalWidth : 0
        }));
    ...
    console.log(`[${i * 300}ms]`, JSON.stringify(state.result.value));
  }
