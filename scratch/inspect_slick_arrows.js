...
async function run() {
  ...
  const res = await send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const next = document.querySelector('button.slick-next');
      if (!prev) return 'no prev arrow';
      ...
      return {
        ...
        h: cs.height,
        display: cs.display,
        innerHTML: prev.innerHTML
      };
    })()`,
    returnByValue: true
  });

  console.log('Slick arrow computed:', JSON.stringify(res.result.value, null, 2));
  ws.close();
  child.kill();
}
