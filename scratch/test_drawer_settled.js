...
async function run() {
  ...
  // Wait 3 seconds for transitions to completely finish
  await wait(3000);
  ...
  const finalState = await send('Runtime.evaluate', {
    expression: `(() => {
      ...
      return {
        ...
        innerRect: { left: r.left, right: r.right, width: r.width },
  ...
  console.log('Final State after 3s:', finalState.result.value);
