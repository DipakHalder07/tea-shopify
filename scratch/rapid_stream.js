...
async function run() {
  ...
  // Navigate
  send('Page.navigate', { url: 'http://127.0.0.1:9292/' });

  // Rapid capture loop starting right away
  for (let i = 0; i <= 20; i++) {
    await wait(150);
    const snap = await send('Page.captureScreenshot');
    if (snap && snap.data) {
      fs.writeFileSync(path.join(outDir, `stream_${i * 150}ms.png`), Buffer.from(snap.data, 'base64'));
    }
  }
