...
async function run() {
  ...
  // Extract CSS from scratch/luxury_styles.css
  const css = `
  ...
  let sAvail = await send('Page.captureScreenshot');
  fs.writeFileSync('scratch/sub_avail.png', Buffer.from(sAvail.data, 'base64'));
  ...
  // 4. Click Price submenu
  await send('Runtime.evaluate', {
    expression: `(() => {
      const summaries = Array.from(document.querySelectorAll('.mobile-facets__summary'));
      const price = summaries.find(s => s.innerText.includes('Price'));
      if (price) price.click();
    })()`
  });
  await wait(500);

  let sPrice = await send('Page.captureScreenshot');
  fs.writeFileSync('scratch/sub_price.png', Buffer.from(sPrice.data, 'base64'));
  ...
  let sClosed = await send('Page.captureScreenshot');
  fs.writeFileSync('scratch/after_close.png', Buffer.from(sClosed.data, 'base64'));
